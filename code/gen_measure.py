...
_LBT_THROUGHPUT_STEMS = {
    ...
    "armadd", "armsub", "armmove", "armmfflag", "armmtflag",
    "setarmj",
}
                ...
                # ── LBT ─────────────────────────────────────────────
                    ...
                    if stem in _LBT_SETUP_LATENCY_STEMS and gpr_count == 2:
                        ...
                        asm = _lbt_emit_asm(
                            ...
                            stem,
                            lbt_parts,
                            lambda gpr_idx, _lane: _LAT if gpr_idx == 0 else "$r13",
                            lane=-1,
                        )
                    ...
                    if stem in _LBT_LATENCY_STEMS:
                        def _lat_reg(gpr_idx, _lane):
                            return _LAT if gpr_idx <= 1 else "$r13"

                        asm = _lbt_emit_asm(name, stem, lbt_parts, _lat_reg, lane=-1)
                        clob = '"r12", "r13"' if gpr_count >= 3 else '"r12"'
                        col.instr_test("lbt", f"{safe_name}_0", f'"{asm}\\n"', clob)
